    ...
    : QWidget(parent)
{
    ...
    m_lsystemEdit->setPlainText(QStringLiteral(
      ...
      "#{Stripe, frequency, lineThickness, [onValue], [offValue]} on any channel slot\n"
      "#{Noise, scale, [octaves], [seed], [minValue], [maxValue]} for roughness/metallic variation\n"
      "# grid-lamp: put {Grid,...} in the emission slot; lines emit (mask 1), cells dark (mask 0)\n"
