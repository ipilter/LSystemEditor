...
{
    LobeWeights weights{};
    if (materialIsClearMedium(ctx.material)) {
        weights.diffuse = 0.0f;
        weights.specular = 1.0f;
        return weights;
    }
...
PRINCIPLED_BRDF_FN float interfaceFresnelReflectance(const BrdfContext& ctx)
{
    ...
    const float ior = materialIor(ctx);
    const bool insideMedium = BrdfDetail::brdfEtaDiffersFromAir(ctx.etaMedium);
    const float etaT = insideMedium ? BrdfDetail::kAirIor : ior;
    ...
    float& nextMediumEta)
{
    ...
    const bool insideMedium = BrdfDetail::brdfEtaDiffersFromAir(ctx.etaMedium);
    if (insideMedium) {
        etaT = BrdfDetail::kAirIor;
        nextMediumEta = BrdfDetail::kAirIor;
    } else {
        etaT = ior;
        nextMediumEta = ior;
    }
