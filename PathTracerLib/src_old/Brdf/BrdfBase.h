...
/** @brief Relative IOR delta above air treated as a distinct medium (ior > 1 + 1e-6). */
constexpr float kMediumIorRelativeEpsilon = 1.0e-6f;

BRDF_BASE_FN bool brdfEtaDiffersFromAir(float eta)
{
    return (eta - kAirIor) > kMediumIorRelativeEpsilon * kAirIor;
}
