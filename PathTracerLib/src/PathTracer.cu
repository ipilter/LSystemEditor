...
__global__ void writeEmissiveLightsDebugToPboKernel(
    const CameraGpu* camera,
    const MeshAccelSceneGpu* scene,
    uchar4* pbo,
    int width,
    ...
    const RenderParamsGpu* params)
{
    ...
    const MeshHit hit = meshAccelTraceRay(
        ...
        0.001f,
        1.0e30f);
    if (hit.hit && lightTriangleIsEmissive(scene, hit.triangleIndex)) {
        rgb = make_float3(1.0f, 0.65f, 0.15f);
    }
...
bool pathTracerWriteEmissiveLightsDebugToPbo(
    const CameraGpu* d_camera,
    const MeshAccelSceneGpu* d_scene,
    uchar4* pbo,
    int width,
    ...
    cudaStream_t stream)
{
    ...
    const dim3 block(16, 16);
    const dim3 grid = grid2d(width, height, block);
    writeEmissiveLightsDebugToPboKernel<<<grid, block, 0, stream>>>(
        d_camera, d_scene, pbo, width, height, d_params);
    return checkLaunch(cudaSuccess);
}
